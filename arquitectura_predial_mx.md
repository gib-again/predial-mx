...
| Estados objetivo | 10–15 (11 completados/en proceso) |
| Catálogo de municipios | INEGI, disponible para todo México |
| LLM | API de GPT-5.2 (OpenAI Chat Completions con Structured Output) |
| OCR | ocrmypdf + Tesseract local (solo para estados con PDFs escaneados) |
...
│       ├── guanajuato/
│       │   ├── __init__.py
│       │   ├── config.py           # 46 municipios, aliases OCR, API endpoints
│       │   ├── download.py         # API REST backperiodico.guanajuato.gob.mx
│       │   ├── ocr.py              # OCR con ocrmypdf --force-ocr (cross-platform)
│       │   └── segment.py          # Segmentación 2 niveles (leyes → predial)
│
...
| Guanajuato | `GTO` | `GTO_2020_leon.json` |
...
|------|---------------|-------------|
| `tarifa_millar` | Tasa(s) al millar diferenciadas por tipo de predio. Puede incluir cuota fija adicional. | `tabla_tarifa_millar` (con `cuota_fija_adicional` nullable) |
...
|------|---------------------------------------------|
| `tarifa_millar` | `V × tasa_millar / 1000 + cuota_fija_adicional.monto` (para el grupo relevante) |
